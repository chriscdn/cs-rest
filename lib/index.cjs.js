...
var lodash$1 = _interopDefault(require('lodash.isnil'));

var formDataFactory = {
	createFormData() {
		 {
			const klass = formData;
			return new klass
		}		
	}
};
	...
	if (otcsticket) {
		...
		instance.interceptors.request.use(async request => {
			...
			if (request.headers.common['OTCSTicket']) {
				...
				let response =  await axios.post(`${options.baseURL}/api/v1/auth/`, formData.getBuffer(), { headers: formData.getHeaders() }) ;
...
var nodes = session => ({
	...
	async addDocument(parent_id, fileHandler, fileName = null) {
...

		{
			...
			let name = fileName || path$1.basename(fileHandler);
	...
	async addVersion(dataid, fileHandler, options = {}) {
...

		{
			...
			const params = {
				file: {
					file: f,
					name: path$1.basename(fileHandler)
				},
				...options
			};
...
var Session_1 = class Session {
	...
	putForm(url, params) {
		...
		return  this.put(url, formData.getBuffer(), { headers: formData.getHeaders() }) 
	}

	postForm(url, params) {
		const formData = this._objectToForm(params);
		return  this.post(url, formData.getBuffer(), { headers: formData.getHeaders() }) 
	}

	patchForm(url, params) {
		const formData = this._objectToForm(params);
		return  this.patch(url, formData.getBuffer(), { headers: formData.getHeaders() }) 
	}
