(function (global, factory) {
	...
	var isObject$2 = /*@__PURE__*/getDefaultExportFromCjs(isObject$1);
	...
	class RPCError extends Error {
	  constructor(message = ErrorCodes.INTERNALERROR.message, data = null, code = ErrorCodes.INTERNALERROR.code) {
	    var __super = (...args) => {
	      super(...args);
	      __publicField$5(this, "code");
	      __publicField$5(this, "data");
	      return this;
	    };
	    if (isObject$2(message)) {
	      const messageObj = message;
	      __super(messageObj.message);
	...
	const getInstance = (options) => {
	  ...
	  instance.interceptors.response.use(
	    (response) => {
	      const otcsticket = get$1(response, "headers.otcsticket");
	      if (otcsticket) {
	  ...
	  if (otcsticket) {
	    ...
	    instance.interceptors.request.use(async (config) => {
	      if (config.headers.common?.OTCSTicket) {
	        ...
	        config.headers.OTCSTicket = get$1(response, "data.ticket");
	        return config;
	      }
	    });
	  } else {
	...
	class WorkflowInitiator {
	  constructor(session, mapId) {
	    ...
	    this.mapId = mapId;
	    __publicField$3(this, "workflowPropertiesInfo", null);
	  }
	  /**
	  ...
	  findWorkflowAttribute(attributeName) {
	    const properties = this.form.schema.properties;
	  ...
	  get wantAuthentication() {
	    return this.workflowPropertiesInfo.data.authentication;
	  }
	  setWorkflowAttribute(attributeName, value) {
	...
	class Workflow extends ServiceAbstract {
	  workflowInitiator(mapId) {
	    return new WorkflowInitiator(this._session.deref(), mapId);
	  }
	  start(mapId) {
	    return this.draftprocesses(mapId).then((draftProcess) => draftProcess.results.draftprocess_id).then(
	      (draftprocessId) => this.draftprocessesUpdate(draftprocessId)
	    );
	  }
	  async draftprocesses(workflowId) {
	    const { data } = await this.session.postForm("api/v2/draftprocesses", {
	      workflow_id: workflowId
	    });
	    return data;
	  }
	  async draftprocessesUpdate(draftprocessId) {
	  ...
	  async draftprocessesPut(draftprocessId, body) {
	...
	var isnil = /*@__PURE__*/getDefaultExportFromCjs(lodash_isnil);
	...
	class RPCClient {
	  constructor(session, relativePath) {
	    __publicField$1(this, "session");
	    __publicField$1(this, "relativePath");
	    /* protected */
	    __publicField$1(this, "_batchQueue");
	    this.session = session;
	...
	class Session {
	  constructor(options) {
	    ...
	    __publicField(this, "_workflow");
	    __publicField(this, "_rhcore");
	    __publicField(this, "_members");
	    __publicField(this, "_search");
	    __publicField(this, "_webreports");
	...
	const isRPCError = (e) => {
	  return e.code !== void 0 && e.data !== void 0;
	};
