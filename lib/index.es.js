...
function axiosFactory(options) {
	...
	const username = get$1(options, 'username');
	const password = get$1(options, 'password');
	const otcsticket = get$1(options, 'otcsticket');
	...
	if (otcsticket) {
		...
		instance.interceptors.request.use(async request => {
			...
			if (request.headers.common['OTCSTicket']) {
				...
				request.headers.common['OTCSTicket'] = get$1(response, 'data.ticket');
...
var rpcClient = class RPCClient {
	...
	async batch() {
		...
		return get$1(response, 'data.data', []).map(item => this.handleResponse(item))
	}
